...
#include "../ProcessBlocks/IQFFT.hpp"
...
Err IQFFT::init(vector<tVar>& args)
{
	...
	m_pGraph->axis(Fs/(m_samples * 2.0), -30, Fs, 60, 2, 0);
...
Err IQFFT::process()
{
	if (m_pGraph->isDirty() == false)
	{
		...
		IQSample*   src = m_in->data();
		SampleType* dst = arr.data();
		for (int i=0; i<m_samples; ++i)
		{
			dst[i*2]   = real(src[i]);
			dst[i*2+1] = imag(src[i]);
		}
		tSimpleFFT(dst, m_samples, -1);

		arr.resize( arr.size() / 2);
		unsigned int mid = floor( arr.size() / 2 );
		for (unsigned int i=0; i<arr.size(); ++i)
		{
			if (i < mid)
				dst[i+mid] = 20.0 * log10( abs(complex<double>(dst[i*2],dst[i*2+1])) );
			else
				dst[i-mid] = 20.0 * log10( abs(complex<double>(dst[i*2],dst[i*2+1])) );
		}
...
void IQFFT::tSimpleFFT(SampleType *fftBuffer, long fftFrameSize, short sign)
/*
